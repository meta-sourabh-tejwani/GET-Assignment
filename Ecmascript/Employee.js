...
class Employee {
    ...
    constructor(name, gender, mail, password, contact) {
        this.name = name;
        this.gender = gender;
        this.mail = mail;
...
function setEmployeeName(textbox) {
    if (/^[a-zA-Z ]*$/.test(textbox.value) == false) {
        ...
        let text = "Hii " + employee_name + " Can I Know your gender";
...
function setGender(textbox) {
    ...
    let text = "Hii " + employee_name + " Can I Know your mail-id";
    document.getElementById("name-label-test").innerHTML = text;
}
...
function setEmployeeMail(textbox) {
    if (/\S+@\S+\.\S+/.test(textbox.value)) {
        ...
        let text = "Hii " + employee_name + " Enter  Strong password";
        document.getElementById("name-label-test").innerHTML = text;
    }
    else {
...
function setPassword(textbox) {
    if (textbox.value.length < 8) {
        ...
        let text = "Hii " + employee_name + " Please Enter password one more time to Verify password";
        document.getElementById("name-label-test").innerHTML = text;
    }
...
function confirmPassword(textbox) {
    if (textbox.value == employee_password) {
        ...
        let text = "Hii " + employee_name + " Can I Know your Contact number";
        document.getElementById("name-label-test").innerHTML = text
    }
    else {
...
const employee = [];
let employee_name, employee_gender, email, employee_password, confirm_password, contact_number;



function setEmployee() {
    employee.push(new Employee(employee_name, employee_gender, mail, employee_password, contact_number));
    document.getElementById("employee-form").style.display = "none";
    let text = "Employee id-" + employee[0].id;
